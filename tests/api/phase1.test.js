...
// both roles, legacy /auth aliases, plan lookup normalisation, gyms, migrations.
...
function register(agent, role, overrides = {}) {
...
describe.each(['trainee', 'trainer'])('%s account lifecycle', role => {
  ...
  test('logs in and stores user_id/email/role/name in the session', async () => {
    const res = await agent.post('/api/auth/login').send({ email: `${role}${DOMAIN}`, password: PASSWORD });
    ...
    expect(session.body.id).toBe(session.body.user_id);
  });
  ...
  test('updates the role-specific profile', async () => {
    ...
    const res = await agent.patch('/api/profile').send(body);
    expect(res.status).toBe(200);
  ...
  test('legacy POST /auth/update-profile works and validates input', async () => {
    const ok = await agent.post('/auth/update-profile').send({ location: 'Koramangala' });
    expect(ok.status).toBe(200);
    expect(ok.body.profile.location).toBe('Koramangala');
    const bad = await agent.post('/auth/update-profile').send(role === 'trainee' ? { age: 'abc' } : { experience: -3 });
...
describe('auth validation and legacy aliases', () => {
  test('rejects an invalid role', async () => {
    const res = await register(request(app), 'coach', { email: `coach${DOMAIN}` });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/trainer.*trainee/);
  });

  test('rejects mismatched passwords', async () => {
    const res = await register(request(app), 'trainee', { email: `mismatch${DOMAIN}`, confirmPassword: 'nope-nope-nope' });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('Passwords do not match');
  });

  test('unknown email and wrong password give the same error', async () => {
    const a = await request(app).post('/api/auth/login').send({ email: `nobody${DOMAIN}`, password: 'x' });
    const b = await request(app).post('/api/auth/login').send({ email: `trainee${DOMAIN}`, password: 'wrong' });
    expect(a.status).toBe(400);
    expect(a.body.error).toBe(b.body.error);
  });

  test('legacy /auth/login, /auth/session (GET+POST) and GET /auth/logout still work', async () => {
    const agent = request.agent(app);
    expect((await agent.post('/auth/login').send({ email: `trainer${DOMAIN}`, password: PASSWORD })).status).toBe(200);
    expect((await agent.get('/auth/session')).body.loggedIn).toBe(true);
    expect((await agent.post('/auth/session')).body.loggedIn).toBe(true);
    expect((await agent.get('/auth/logout')).status).toBe(200);
    expect((await agent.get('/auth/session')).body.loggedIn).toBe(false);
  });
  ...
  test('/home serves the dashboard only when logged in', async () => {
    ...
    const agent = request.agent(app);
    await agent.post('/api/auth/login').send({ email: `trainee${DOMAIN}`, password: PASSWORD });
...
describe('curated plans and gyms', () => {
  ...
  test('gyms: case-insensitive, Bengaluru/Bangalore alias, maps_url fallback', async () => {
    ...
    expect((await request(app).get('/auth/gyms/bangalore')).body.some(g => g.name === 'JEST Iron Temple')).toBe(true);
  });
});
